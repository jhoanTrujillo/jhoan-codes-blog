...
The majority of validations were fine and didn't have major issues other than trailing forward slashes, or missing orphan children that needed to be corrected. But the post_view page, since it is using the summernote Django plugin to add content to the blogs, seems to be causing issues with validation as the code generated by the rich text editor clashes with what is expected by the validator.

This is a bug that might not be worth fixing as it is caused by third-party tools that auto generate code based on the text that was inputted.
...
Some of the bugs found in the project can be seen and documented in the project board as issues.
